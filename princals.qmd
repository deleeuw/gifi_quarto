...
h <- princals(epi.bfi, epi_knots, epi_degrees, epi_ordinal, epi_copies)
```
```{r eigen_epi, echo = FALSE}
val0 <- eigen(cor(epi.bfi))$values
...
h <- princals(epi.bfi, knotsE(epi.bfi), epi_degrees, epi_ordinal)
```
...
for (j in 1:13) {
  oj <- order (epi.bfi[,j])
  plot(epi.bfi[oj,j], h$transform[[j]][oj], col="RED", xlab=lbs[j], ylab="transform", type = "l", lwd = 3)
}
par(mfrow=c(1,1))
```
<center>
